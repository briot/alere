...
fn round_and_err(d: Decimal, strat: RoundingStrategy) -> (Decimal, Decimal) {
    // Round to 0 decimal places
    let v = d.round_dp_with_strategy(0, strat);
    let err = if d.is_zero() {
        Decimal::ZERO
    }
...
pub fn scale_value(value: Option<Decimal>, scale: ScalingFactor) -> Option<i64> {
    let v = match value {
        None => return None,
        Some(v) => v,
    };
    ...
    let (d2, err2) = round_and_err(scaled, RoundingStrategy::MidpointAwayFromZero);
    ...
    if err > Decimal::from_f32_retain(0.001).unwrap() {
        println!(
            "WARNING: price {} \
                ...
                scale={}",
            v,
            pdec.to_i64().unwrap_or(0) / (scale as i64),
