...
def cte_list_splits(
    ...
    non_recurring_splits = f"""
        ...
        WHERE t.scheduled IS NULL
            ...
            AND post_date >= '{dates.datemin}'
            AND post_date <= '{dates.datemax}'
    """
    ...
    if max_occurrences > 0:
        ...
        WHERE s.post_date IS NOT NULL AND s.post_date < '{dates.datemax}'
    ...
    return f"""
       ...
       WHERE
          ...
          strftime("%Y-%m-%d", {CTE_BALANCES_CURRENCY}.mindate)
             <= strftime("%Y-%m-%d", {DateSet.CTE}.date)
          AND strftime("%Y-%m-%d", {DateSet.CTE}.date)
...
def networth_history(
    ...
    query = f"""
        ...
        SELECT
           ...
           AVG(tmp2.diff) OVER
               (ORDER BY tmp2.date
                ROWS BETWEEN {dates.prior:d} PRECEDING
                AND {dates.after:d} FOLLOWING) AS average,
           tmp2.value
        FROM
           (SELECT
              tmp.date,
    ...
    query = f"""
        ...
        SELECT
           ...
           AVG(inc_total) OVER
               (ORDER BY tmp.month
                ROWS BETWEEN {dates.prior:d} PRECEDING
                AND {dates.after:d} FOLLOWING) AS inc_average,
           exp_total,
           AVG(exp_total) OVER
               (ORDER BY tmp.month
                ROWS BETWEEN {dates.prior:d} PRECEDING
                AND {dates.after:d} FOLLOWING) AS exp_average
        FROM
           (
              --  Sum of splits for a given months, organized per
              --  category
              SELECT
                 strftime("%Y-%m", {DateSet.CTE}.date) as month,
