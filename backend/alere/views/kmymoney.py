...
class KMyMoney:
    ...
    def accounts(self):
        ...
        List all accounts
        """
        return pd.read_sql_query(self.query_accounts(), self.sqlite)
    ...
    def networth(
        ...
        if by_year:
            ...
            inc = "+1 MONTHS"     # or date(s.postDate, 'start of month', '+1 month')
            formatted = "%Y-%m"   # or date(s.postDate, 'start of month')
    ...
    def query_ledger(
        ...
        return (
            ...
            SELECT
               ...
               {self._to_float('destS.value')} as value,
    ...
    def ledger(
        ...
        q, params = self.query_ledger(accounts, currency, mindate, maxdate)
        return pd.read_sql_query(q, self.sqlite, params)
    ...
    def plot_by_category(
        ...
        account_types=(ACCOUNT_TYPE.INCOME, ACCOUNT_TYPE.EXPENSE),
        ...
        p = pd.read_sql(
            ...
            FROM ({q}) s
               ...
               JOIN kmmAccounts destAccount
                  ON (destS.accountId = destAccount.id)
            WHERE TRUE{test_min_date}
               AND destAccount.accountType IN ({','.join(str(s) for s in account_types)})
            """,
            self.sqlite,
            params={
                "mindate": mindate,
